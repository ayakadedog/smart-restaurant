...
<!--      上面搜索名称-->
      <div class="tableBar">
        <label style="margin-right: 10px">桌号：</label>
        <el-input v-model="inputTableNumber"
                  placeholder="请填写桌号"
                  style="width: 14%"
                  clearable
                  @clear="init"
                  @keyup.enter.native="initFun" />
        <label style="margin-right: 10px; margin-left: 20px">座位数量：</label>
        <el-input v-model="inputSeatNumber"
                  placeholder="请填写座位数"
                  style="width: 14%"
                  clearable
                  @clear="init"
                  @keyup.enter.native="initFun" />
        <label style="margin-right: 10px; margin-left: 20px">地点：</label>
        <el-input v-model="inputArea"
                  placeholder="请填写地点"
                  style="width: 14%"
                  clearable
                  @clear="init"
                  @keyup.enter.native="initFun" />
<!--        <label style="margin-right: 10px; margin-left: 20px">材料：</label>-->
<!--        <el-input v-model="inputTableType"-->
<!--                  placeholder="请填写材料"-->
<!--                  style="width: 14%"-->
<!--                  clearable-->
<!--                  @clear="init"-->
...
<!--                @click="deleteHandle('批量')">批量删除</span>-->
      ...
      <el-table v-if="tableData.length"
        ...
        <el-table-column type="selection"
                         width="25" />
        <el-table-column prop="tableNumber"
                         label="桌号" />
...
<!--          </template>-->
<!--        </el-table-column>-->
        <el-table-column prop="seatNumber"
                         label="座位容纳人数" />
...
import { deleteTableById, getTablePage } from '../../api/table'
...
export default class extends Vue {
  private inputTableNumber: any = ''
  private inputSeatNumber: any = ''
  private inputArea: any = ''
  private inputTableType: any = ''
  ...
  private async init(isSearch?) {
    ...
    await getTablePage({
      ...
      tableNumber: this.inputTableNumber || undefined,
      seatNumber: this.inputSeatNumber || undefined,
      area: this.inputArea || undefined,
      tableType: this.inputTableType || undefined,
    })
      .then(res => {
  ...
  // 添加更改
  private addSetTable(st: any) {
    if (st === 'add') {
      this.$router.push({ path: '/table/add' })
    } else {
      this.$router.push({ path: '/table/add', query: { id: st.id } })
    }
  }
