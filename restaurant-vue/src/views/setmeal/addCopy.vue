      ...
      name: {
        ...
        validator: (rule: any, value: string, callback: Function) => {
          if (!value) {
            ...
            const text = this.ruleForm.name

            // 使用正则表达式替换文本，确保只有中文汉字和英文单词之间有空格
            const ans = text.replace(/([A-Za-z]+)|([\u4e00-\u9fa5]+)/g, (match, word, chinese) => {
            if (word) {
                return ` ${word} `;
            } else if (chinese) {
                return chinese;
            ...
            // 移除多余的空格
            const finalText = ans.replace(/\s+/g, ' ').trim()

            // 移除中文汉字和中文汉字之间的空格
            const finalTextRemoveSpace = finalText.replace(/([\u4e00-\u9fa5])\s+([\u4e00-\u9fa5])/g, '$1$2');

            // 更新输出元素
            this.ruleForm.name = finalTextRemoveSpace
          ...
          addSetmeal(prams)
            .then(res => {
              if (res && res.data && res.data.code === 1) {
                ...
                if (!st) {
                  this.$router.push({ path: '/setmealMost' })
                } else {
